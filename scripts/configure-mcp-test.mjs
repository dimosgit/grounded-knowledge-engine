...
try {
  ...
  assert.ok(projectServer.args.length === 1 || projectServer.args.length === 2);
  assert.deepEqual(vscodeServer.args, projectServer.args);
  assert.equal(projectServer.env.KB_MCP_PROFILE, "core");
  assert.equal(projectServer.env.KB_MCP_REPO_ROOT, configRoot);
  ...
  const geminiSettings = readJson(join(".gemini", "settings.json"));
  const geminiServer = geminiSettings.mcpServers.kb;
  ...
  assert.equal(geminiServer.env.KB_MCP_REPO_ROOT, configRoot);
  assert.equal(geminiServer.env.KB_MCP_ENABLE_WRITES, "true");
  assert.equal(geminiServer.cwd, configRoot);
