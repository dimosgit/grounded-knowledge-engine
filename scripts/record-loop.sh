...
# current real loops — grounded retrieval, capture/reuse, and structured project
# resume. Runs the actual engine; no hand-typing, no retakes.
#
...
# The narrated body: three real commands with colored banners.
cat > "$SCRIPT" <<'BODY'
set -e
printf '\033[1;36m# Grounded Knowledge Engine — knowledge that survives the chat\033[0m\n\n'
...
npm run search --silent -- \
  ...
  --mode generic --limit 1 --context 0 | fold -s -w 108
printf '\n\033[1;32m$ prove MCP discovery, resources, answer -> capture -> re-answer\033[0m\n'
npm run smoke:mcp --silent
printf '\n\033[1;32m$ resume one explicit project (same facts in MCP + Cockpit)\033[0m\n'
npm run demo:project --silent -- router-rollout | fold -s -w 108
printf '\n\033[1;36m# local files -> cited answers -> durable learning -> project handoff\033[0m\n'
BODY

# 1) Record (headless, fixed width so nothing wraps, idle capped to trim startup pauses).
asciinema rec --headless --window-size 116x42 -i 1.2 -c "bash '$SCRIPT'" "$CAST" --overwrite
...
npx --yes svg-term-cli --in "$CAST_V2" --out docs/loop.svg --window --no-cursor --at 5000
