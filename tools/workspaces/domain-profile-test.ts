...
const SPECIALIZED: WorkspaceDomainConfig = {
  label: "SAP",
  modeAliases: { sap: "domain", client-alpha: "project" },
  queryExpansions: {
    merge: "replace",
  ...
  pathMappings: [
    ...
    { prefix: "client-alpha/", sourceKind: "project", track: "sap" },
    { prefix: "kb/", track: "sap" },
  ],
  defaultTrack: "sap",
  inferMode: {
    project: ["\\bclient-alpha\\b|\\btask\\s*\\d+\\b"],
    domain: ["\\brap\\b|\\babap\\b|\\beml\\b"],
  },
  projectQueryPattern: "\\bclient-alpha\\b",
  scoringRules: [
    {
      id: "book_priority",
      backend: "sqlite",
    ...
    { id: "module_pref", mode: "project", module: "client-alpha-cr-so", boost: 2.1 },
  ],
  captureDefaults: {
    track: "sap",
    module: "rap-core",
  ...
  projectModeModule: "client-alpha-cr-so",
...
function testDefaultProfileReproducesHistoricalBehavior(): void {
  ...
  assert.equal(profile.projectQueryPattern.test("about client-alpha"), false);
...
function testSpecializedProfile(): void {
  ...
  assert.equal(resolveModeAlias(profile, "CLIENT-ALPHA"), "project");
  assert.deepEqual(advertisedModeNames(profile), ["auto", "sap", "client-alpha", "generic"]);
  ...
  assert.equal(profile.projectQueryPattern.test("client-alpha approval flow"), true);
