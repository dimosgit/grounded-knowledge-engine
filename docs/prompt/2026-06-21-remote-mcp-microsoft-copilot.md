...
Implemented: shared `stdio`/HTTP dispatch, loopback-only authenticated
Streamable HTTP, forced read-only discovery and execution, remote-safe
citations, request limits, HTTP integration coverage, the local GitHub Copilot
adapter, Copilot Studio onboarding, and a host compatibility matrix.

Still planned: live ngrok-to-Copilot Studio validation in an approved tenant,
OAuth/OIDC or Entra authorization for a production Microsoft boundary, and the
Toolkit-generated Microsoft 365 declarative-agent package. The repository
contains a validation recipe rather than presenting an untested, manually
authored package as complete.

## 3. Context

- Product area: `MCP transport, deployment, authentication, setup adapters, and enterprise integration`
- Current behavior: `GKE runs through local stdio for Claude Code, Codex, Gemini CLI, and GitHub Copilot, and exposes the same read-only application layer through an opt-in loopback Streamable HTTP bridge. Microsoft agent validation still requires a temporary reachable HTTPS URL and an approved tenant.`
...
Open question 3 — "which Microsoft tenant and licensing path will be available?" — is not an open question, it is a blocker. Copilot Studio / M365 declarative agents need a tenant with the right licensing. Most technical users **cannot click and reproduce this work**; at best they see a screenshot and a configuration sample. This feature delivers a compatibility _matrix_ (Req 19) and a _guide_ (Req 16) for an integration most viewers cannot verify. That is a badge, not product proof.
...
| Public product-proof signal | 2     | Enterprise integration is credible only when a qualified user can run it end to end.           |
...
- **Valuable follow-up (someday):** a private self-hosted Streamable HTTP gateway on the SDK, read-only, single workspace, API-key auth — _if and only if_ a real client/tenant materializes (Open question 1/3).
