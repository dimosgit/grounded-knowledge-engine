  ...
  return (
    ...
    >
      <div className="flex min-h-[calc(100vh-4rem)] flex-col bg-surface-main">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-2 text-body-md text-on-surface-variant">
                Explore the major workspace map: tracks, modules, clients, and projects. Choose a
                focus to show only its connected major contexts.
              </p>
            </div>
        ...
        <div
          className={`grid flex-1 grid-cols-1 ${isGraphLinksCollapsed ? "lg:grid-cols-[minmax(0,1fr)_76px]" : "lg:grid-cols-[minmax(0,1fr)_360px]"}`}
        >
          <section
            ...
            >
              ...
              {displayedGraphNodes.map((node) => (
                ...
                >
                  ...
                  {node.summary && (
                    <div className="mt-2 font-mono text-code-sm opacity-70">{node.summary}</div>
                  )}
                </button>
              ))}
              ...
              {!contextGraph.relationships.length && (
                <div className="absolute left-1/2 top-1/2 max-w-md -translate-x-1/2 translate-y-20 rounded-lg border border-border-subtle bg-surface-container p-5 text-center text-body-md text-on-surface-variant">
                  No major context links found for this focus yet. Add module ownership or project
                  module metadata to connect it.
                </div>
              )}
            </div>
          </section>
          ...
          <aside className={`bg-surface-sidebar ${isGraphLinksCollapsed ? "p-3" : "p-5"}`}>
            ...
            >
              {!isGraphLinksCollapsed && (
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-metadata text-on-surface-variant">
                    {contextGraph.nodes.length} nodes and {contextGraph.edges.length} links in this
                    view.
                  </p>
                </div>
              )}
              ...
              <button
                ...
                aria-label={
                  isGraphLinksCollapsed
                    ? "Expand major context links"
                    : "Collapse major context links"
                }
                title={
                  isGraphLinksCollapsed
                    ? "Expand major context links"
